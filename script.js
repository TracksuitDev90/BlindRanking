...
(() => {
  ...
  // Pre-vetted image manifest (images.js, generated by tools/build-images).
  // An item only ever shows an image that the build pipeline resolved,
  // validated and (where needed) a human approved; everything else renders
  // as text — a wrong image is worse than no image.
  function manifestEntryFor(item, topic) {
    const images = window.BR_IMAGES;
    if (!images || !item?.label) return null;
    const labelKey = normalize(item.label).replace(/ /g, '-');
    if (topic?.name) {
      const scoped = images[`${labelKey}@@${normalize(topic.name).replace(/ /g, '-')}`];
      if (scoped) return scoped;
    }
    return images[labelKey] || null;
  }

  function applyManifestStyle(imgEl, entry) {
    imgEl.style.objectFit = entry.f || 'contain';
    imgEl.style.objectPosition = `${entry.x ?? 50}% ${entry.y ?? 50}%`;
    imgEl.style.padding = entry.p ? '16px' : '0';
  }

  function loadManifestImage(imgEl, url) {
    return new Promise((resolve, reject) => {
      const temp = new Image();
      temp.crossOrigin = 'anonymous';
      temp.onload = () => {
  ...
  async function resolveAndRender(item) {
    ...
    const entry = manifestEntryFor(item, App.currentTopic());
    ...
    if (entry) {
      ...
      try {
        applyManifestStyle(img, entry);
        await loadManifestImage(img, entry.u);
        if (myToken !== paintToken) return false;
        currentResolvedURL = entry.u;
        prefetchNext();
        setBusy(false);
        App.persist();
        return true;
      } catch (_) {
        // Vetted URL failed to load (network/CDN hiccup) — fall back to text.
        if (myToken !== paintToken) return false;
        if (img) { img.hidden = true; img.removeAttribute('src'); }
      }
    }

    // ——— Text fallback: label as bold centred text ———
  ...
  function prefetchNext() {
    ...
    if (!next) return;
    const entry = manifestEntryFor(next, App.currentTopic());
    if (!entry) return;
    const img = new Image();
